...
export class Throne {
  ...
  settleStanding(day: number) {
    ...
    const lines = dawnStanding(standing, e, this.host.memory.reputation({ kind: 'authority' }).trust, day, this.host.sim.content)
  ...
  decree(d: Decree, proclaim = true): DecreeResult {
    ...
    const result = enact(e, d, this.host.sim.content.realm!)
  ...
  setRulerMode(rulerMode: RulerMode) {
    ...
    if (rulerMode === 'model' && useTown.getState().llm.active === 'mock')
      useTown.getState().toast(`No hay un modelo configurado: ${RULER.title} gobernará con reglas hasta que elijas uno en Configuración.`)
  }
  ...
  async reign(force = false) {
    ...
    if (keyless) state.toast(`${RULER.Title} gobierna hoy con reglas: falta la key del modelo.`)
    const useModel = state.rulerMode === 'model' && state.llm.active !== 'mock' && !keyless
    const modelOk = useModel && state.rulerCalls < state.rulerCap
    if (useModel && !modelOk) state.toast(`${RULER.Title} llegó al tope de ${state.rulerCap} consultas al modelo en esta partida; gobierna con reglas.`)
    const active = state.llm.active
    const ruler = modelOk && active !== 'mock' ? createModelRuler(state.llm.connections[active], this.host.sim.content) : createRulesRuler()
    ...
    try {
      ...
      if (reply.actions.length) this.host.log('ruler', `${RULER.Title}: «${clip(reply.thought, 160)}»`)
    } catch (err) {
      ...
      state.toast(`${RULER.Title} no pudo decidir hoy: ${error}`)
    } finally {
  ...
  private async hearPetitions(e: Economy): Promise<Petition[]> {
    ...
    for (const p of petitions) this.host.log('petition', `${p.from} pide ${RULER.to}: «${clip(p.text, 140)}»`)
    return petitions
  }
  ...
  private carryOut(a: RulerAction, day: number, announce = false, via?: string): { text: string; ok: boolean } {
    ...
    if (a.kind === 'ask') {
      ...
      useTown.getState().toast(`📬 ${RULER.Title} te ha escrito una carta.`)
      this.host.log('ruler', `${RULER.Title} escribe al creador: «${clip(a.text, 120)}»`)
