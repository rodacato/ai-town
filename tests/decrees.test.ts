...
describe('decrees', () => {
  it('changes taxes and prices within limits, and says why when it cannot', () => {
    const e = fresh()
    expect(enact(e, { kind: 'tax', rate: 0.3 })).toMatchObject({ ok: true, proclamation: expect.stringContaining('sube') })
    expect(e.taxRate).toBe(0.3)
    expect(enact(e, { kind: 'tax', rate: 0.9 })).toMatchObject({ ok: false, reason: expect.stringContaining('60%') })
    expect(enact(e, { kind: 'tax', rate: 0.3 }).ok).toBe(false)
    expect(enact(e, { kind: 'price', price: 0 })).toMatchObject({ ok: true, proclamation: expect.stringContaining('gratis') })
    expect(enact(e, { kind: 'price', price: 2.5 }).ok).toBe(false)
  })

  it('spends the treasury only when it can afford it', () => {
    const e = fresh()
    e.treasury = 10
    expect(enact(e, { kind: 'buyFood', rations: 30 })).toMatchObject({ ok: false, reason: expect.stringContaining('No alcanza') })
    expect(e.treasury).toBe(10)
    e.treasury = 200
    const g = e.granary
    expect(enact(e, { kind: 'buyFood', rations: 30 }).ok).toBe(true)
    expect(e).toMatchObject({ treasury: 200 - 30 * LIMITS.rationCost, granary: g + 30 })
  })

  it('feeds the hungry from the granary', () => {
    const e = fresh()
    expect(enact(e, { kind: 'handout' }).ok).toBe(false)
    e.needs.pip.daysHungry = 2
    e.needs.grum.daysHungry = 1
    expect(enact(e, { kind: 'handout' })).toMatchObject({ ok: true, summary: expect.stringContaining('2 raciones') })
  ...
  it('rationing doubles how long the granary lasts, at a cost in mood', () => {
    ...
    enact(b, { kind: 'law', law: 'rationing', on: true })
    ...
    expect(enact(b, { kind: 'law', law: 'rationing', on: true }).ok).toBe(false)
  })
  ...
  it('lets a careful ruler carry the town through a winter that would otherwise empty it', () => {
    ...
    enact(ruled, { kind: 'buyFood', rations: 100 })
    enact(ruled, { kind: 'law', law: 'rationing', on: true })
    for (let d = 8; d <= 21; d++) {
      runDay(idle, rules, 'winter', d)
      runDay(ruled, rules, 'winter', d)
      if (foodDays(ruled) < 3 && ruled.treasury >= 60 * LIMITS.rationCost) enact(ruled, { kind: 'buyFood', rations: 60 })
    }
